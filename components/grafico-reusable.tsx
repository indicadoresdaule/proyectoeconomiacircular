...
export function GraficoReusable({
  ...
  // Configurar márgenes según dispositivo
  const margin = isMobile
    ? { top: 20, right: 10, left: 50, bottom: 120 }
    : { top: 30, right: 30, left: 80, bottom: 150 }

  // Limitar cantidad de datos para gráfico de barras (mejor visualización)
  const datosMostrados = esSubcategorias && tipo === "barras" ? datos.slice(0, 15) : datos

  return (
    <div className="w-full bg-white p-4 rounded-lg" style={{ height: isMobile ? "450px" : "550px" }}>
          ...
          <BarChart data={datosMostrados} margin={margin}>
            ...
            <Tooltip
              formatter={(value: number) => {
                ...
                return [`${value.toFixed(2)} (${porcentaje.toFixed(2)}%)`, "Valor"]
              }}
          ...
          <PieChart>
            <Pie
              ...
              label={(entry: any) => {
                ...
                return `${entry.name}: ${entry.value.toFixed(1)} (${porcentaje.toFixed(1)}%)`
              }}
